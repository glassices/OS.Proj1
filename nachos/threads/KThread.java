...
public class KThread {
	...
	for (int i = 0; i < 100; i++){
		testThreads[i] = new KThread(new ScheduleTest()).setName("scheduler test thread");
		testThreads[i].fork();
		ThreadedKernel.scheduler.setPriority(testThreads[i], rand.nextInt(8));
	}
	Machine.interrupt().enable();
	
	for (int i = 0; i < 1000; i++){
		Machine.interrupt().disable();
		ThreadedKernel.scheduler.setPriority(testThreads[rand.nextInt(100)], rand.nextInt(8));
		Machine.interrupt().enable();
		yield();
	}
	*/
	...
	/*
	for (int i = 0; i < 100; i++)
		new KThread(new AlarmTest()).setName("alarm test thread").fork();
    */
    }
